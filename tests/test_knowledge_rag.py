...
检索字段完整性、chunk_size/overlap 参数校验，以及 PDF 元数据的正确性。
...
def test_add_document_pdf_extracts_pages(tmp_path: Path):
    ...
    rag = _rag()
    pdf = _write_pdf(tmp_path / "doc.pdf", ["page one text", "page two text"])
...
def test_add_document_pdf_page_metadata_and_chunk_id(tmp_path: Path):
    ...
    rag = _rag()
    pdf = _write_pdf(tmp_path / "pages.pdf", ["first page", "second page"])
    chunks = rag.add_document(pdf)
...
def test_add_document_pdf_skips_empty_page(tmp_path: Path):
    """无文本可提取的页面被跳过，不产生片段。"""
    rag = _rag()
    pdf = _write_pdf(tmp_path / "emptyp.pdf", ["", "only this page has text"])
    chunks = rag.add_document(pdf)
    assert len(chunks) == 1
...
def test_add_directory_recursive_aggregates(tmp_path: Path):
    ...
    _write_pdf(root / "b.pdf", ["pdf page alpha", "pdf page beta"])
