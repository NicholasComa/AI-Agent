...
def create_app(
    *,
    ...
    Returns:
        一个已注册全部三个接口的、配置好的 :class:`FastAPI` 实例。
    """
    ...
    app = FastAPI(
        title="week01-ai-basics API",
        version=version,
        description=(
            "Day 5 最小服务：/health、/chat、/analyze-requirement。"
            "基于 Day 3 的 LlmClient 与 Day 4 的结构化输出 schema 构建。"
        ),
        lifespan=lifespan,
    )
...
def _resolve_version() -> str:
    """读取包版本；未安装时回退到 ``"0.0.0"``。"""
    try:
        return metadata.version("week01-ai-basics")
...
def _register_routes(
    ...
    async def analyze_requirement(
        ...
        # 由全局处理器映射为 502。
        return AnalyzeRequirementResponse.model_validate_json(result.text)
